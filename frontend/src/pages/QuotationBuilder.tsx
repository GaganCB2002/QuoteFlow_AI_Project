...
import { FileText, Plus, Send, Save, Download, User, Building2, Mail, Phone, MapPin, Hash, X, Sparkles, Zap, RefreshCw, Share2, LogIn } from 'lucide-react';
...
const QuotationBuilder = () => {
  ...
  const [loginPrompt, setLoginPrompt] = useState(!storage.getToken());
    ...
    return (
      <Layout>
        <div className="flex items-center justify-center min-h-[60vh] p-8">
          <div className="max-w-md text-center bg-white rounded-2xl shadow-sm border border-[#e8e2d8] p-10">
            ...
            <h2 className="text-2xl font-extrabold text-gray-900 mb-2">Sign in to Continue</h2>
            <p className="text-sm text-gray-500 mb-6">
              You need to be logged in to create quotations. Please sign in or create a free account.
            </p>
            <div className="flex gap-3 justify-center">
              <button
                onClick={() => navigate('/login?redirect=/quotations/new')}
                className="flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-all text-sm"
              >
                <LogIn size={16} /> Sign In
              </button>
              <button
                onClick={() => navigate('/register')}
                className="flex items-center gap-2 px-6 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-all text-sm"
              >
                Create Account
              </button>
            </div>
            ...
            <div>
              ...
              <div className="flex justify-between mt-8 pt-6 border-t border-[#e8e2d8]">
                ...
                <div className="flex gap-3">
                  <button onClick={() => alert('Quotation saved as draft: ' + quoteNo)} disabled={!canNextStep2()} className="flex items-center gap-2 px-5 py-3 text-gray-700 font-bold text-[13px] rounded-xl border border-[#e8e2d8] hover:bg-gray-50 transition-all disabled:opacity-40">
                    <Save size={16} /> Save Draft
                  </button>
                  <button onClick={() => alert('PDF download feature coming soon. Quotation: ' + quoteNo)} disabled={!canNextStep2()} className="flex items-center gap-2 px-5 py-3 bg-white text-indigo-700 font-bold text-[13px] rounded-xl border-2 border-indigo-200 hover:bg-indigo-50 transition-all disabled:opacity-40">
                  ...
                  <button onClick={() => {
                    const message = encodeURIComponent(`Quotation ${quoteNo} from QuoteFlow AI\n\nCustomer: ${customer.name}\nTotal: ${formatINR(total)}\n\nGenerated by QuoteFlow AI`);
                    window.open(`https://wa.me/?text=${message}`, '_blank');
                  }} disabled={!canNextStep2()} className="flex items-center gap-2 px-6 py-3 bg-brand-gold-600 text-white font-bold text-[13px] rounded-xl hover:bg-brand-gold-700 shadow-lg transition-all disabled:opacity-40">
