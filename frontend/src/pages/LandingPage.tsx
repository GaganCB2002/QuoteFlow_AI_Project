...
import {
  ...
  Calendar, CalendarDays, IndianRupee, Zap, Moon, Sun
} from 'lucide-react';
...
const LandingPage = () => {
  ...
  useEffect(() => {
    document.title = 'QuoteFlow AI — India\'s #1 AI Quotation, Billing, CRM & Marketing Platform';
  }, []);
  ...
  const simulateAIAnalysis = () => {
    ...
    setTimeout(() => {
      ...
      let html = '<div class="mb-2.5 font-bold text-gray-900 dark:text-white">Generated Quotation</div>';
      items.forEach(i => {
        html += `<div class="flex justify-between py-1.5 border-b border-gray-100 dark:border-slate-800/50 text-sm text-gray-600"><span>${i.name} <span class="opacity-50">(${i.desc})</span></span><span class="font-semibold text-gray-800 dark:text-gray-100">₹${i.rate.toLocaleString()}</span></div>`;
      });
      html += `<div class="flex justify-between py-1.5 border-b border-gray-100 dark:border-slate-800/50 text-sm opacity-50"><span>Subtotal</span><span>₹${sub.toLocaleString()}</span></div>`;
      html += `<div class="flex justify-between py-1.5 border-b border-gray-100 dark:border-slate-800/50 text-sm opacity-50"><span>GST @ 18%</span><span>₹${tax.toLocaleString()}</span></div>`;
  ...
  const genWizard = (type: string) => {
    ...
    const tax = Math.round(sub * 0.18);
    const total = Math.round(sub + tax);
    let html = `<div class="mb-2.5 font-bold text-gray-900 dark:text-white">${type.charAt(0).toUpperCase() + type.slice(1)} Quotation</div>`;
    d.items.forEach(i => {
      html += `<div class="flex justify-between py-1.5 border-b border-gray-100 dark:border-slate-800/50 text-sm text-gray-600"><span>${i.name} <span class="opacity-50">(${i.desc})</span></span><span class="font-semibold text-gray-800 dark:text-gray-100">₹${i.rate.toLocaleString()}</span></div>`;
    });
    html += `<div class="flex justify-between py-1.5 border-b border-gray-100 dark:border-slate-800/50 text-sm opacity-50"><span>Subtotal</span><span>₹${sub.toLocaleString()}</span></div>`;
  ...
  return (
    <div onMouseMove={handleMouseMove} className="min-h-screen bg-[#f8f6f3] dark:bg-[#0a0f1c] text-[#3d3d4a] dark:text-gray-200 font-['Inter',system-ui,sans-serif] overflow-x-hidden relative transition-colors duration-300">
      ...
      <div 
        ...
        style={{
          background: theme === 'dark' 
            ? `radial-gradient(circle 800px at ${mousePos.x}px ${mousePos.y}px, rgba(79, 70, 229, 0.15), transparent 80%)`
            : `radial-gradient(circle 800px at ${mousePos.x}px ${mousePos.y}px, rgba(239, 68, 68, 0.05), transparent 80%)`
        }}
      />
      
      {/* Grid Pattern */}
      <div className="absolute inset-0 z-0 pointer-events-none opacity-40 dark:opacity-20" style={{ backgroundImage: theme === 'dark' ? 'radial-gradient(#4f46e5 1px, transparent 1px)' : 'radial-gradient(#9ca3af 1px, transparent 1px)', backgroundSize: '40px 40px' }}></div>
      <style>{`
      ...
      {/* Navbar */}
      <nav aria-label="Main navigation" className={`fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-[#0a0f1c]/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 transition-all duration-200 ${scrolled ? 'shadow-sm' : ''}`}>
        <div className="max-w-[1120px] mx-auto px-5 flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3 md:hidden">
            <button onClick={toggleTheme} className="p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 dark:text-gray-400">
              {theme === 'dark' ? <Sun size={20} /> : <Moon size={20} />}
            </button>
            <button className="flex flex-col gap-1 cursor-pointer bg-none border-none p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-800" onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? 'Close navigation menu' : 'Open navigation menu'} aria-expanded={menuOpen}>
              {menuOpen ? <X size={20} className="text-gray-500 dark:text-gray-400" /> : <Menu size={20} className="text-gray-500 dark:text-gray-400" />}
            </button>
          </div>
          <ul className={`${menuOpen ? 'flex' : 'hidden'} md:flex absolute md:static top-16 left-0 right-0 bg-white dark:bg-slate-900 md:bg-transparent flex-col md:flex-row items-center gap-0.5 p-3 md:p-0 border-b md:border-b-0 border-gray-200 dark:border-slate-800 md:border-none shadow-md md:shadow-none list-none`}>
            {[
              ...
              <li key={link.href} className="w-full md:w-auto">
                <a href={link.href} onClick={(e) => { e.preventDefault(); scrollTo(link.href.slice(1)); }} className="block px-3.5 py-2 text-sm font-medium text-gray-500 dark:text-gray-400 rounded hover:text-gray-900 dark:text-white hover:bg-gray-100 md:inline transition-all">{link.label}</a>
              </li>
            ))}
            <li className="flex items-center gap-1.5 ml-0 md:ml-2 mt-2 md:mt-0 w-full md:w-auto">
              <button onClick={toggleTheme} className="hidden md:flex p-2 mr-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 dark:text-gray-400 transition-all" aria-label="Toggle theme">
                {theme === 'dark' ? <Sun size={18} /> : <Moon size={18} />}
              </button>
              <button onClick={() => navigate('/login')} className="flex-1 md:flex-none text-center px-3.5 py-1.5 text-sm font-medium text-gray-500 dark:text-gray-300 rounded hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800 transition-all">Sign In</button>
      ...
      {/* Hero */}
      <section className="py-[100px] md:py-[100px] md:pb-16 relative z-10 overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-indigo-500/10 dark:bg-indigo-600/20 blur-[120px] rounded-full pointer-events-none animate-float"></div>
        
        <div className="max-w-[1120px] mx-auto px-5 grid md:grid-cols-2 gap-12 md:gap-16 items-center relative">
          <div className="animate-fade-in-up">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-50 dark:bg-indigo-500/10 rounded-full text-xs font-semibold text-indigo-600 dark:text-indigo-400 mb-4 border border-indigo-100 dark:border-indigo-500/20 shadow-sm">
              🏆 India's #1 AI Business Platform
            </div>
            <h1 className="text-[clamp(28px,3.8vw,44px)] font-extrabold leading-[1.15] tracking-tight text-gray-900 dark:text-white mb-3.5">
              Replace 5 Tools With<br /><span className="text-red-500 dark:text-red-400">One AI Platform</span>
            </h1>
            <p className="text-base text-gray-500 dark:text-gray-300 leading-relaxed mb-6 max-w-[500px]">
            ...
            <div className="flex gap-2.5 flex-wrap">
              <button onClick={() => navigate('/login')} className="inline-flex items-center justify-center gap-2 px-[30px] py-3.5 bg-indigo-600 text-white font-semibold text-sm rounded-lg hover:bg-indigo-700 shadow-lg hover:shadow-xl dark:shadow-indigo-500/20 transition-all hover:-translate-y-0.5">
                Start Free &mdash; No Card Needed
              </button>
              <a href="#demo" onClick={(e) => { e.preventDefault(); scrollTo('demo'); }} className="inline-flex items-center justify-center gap-2 px-[30px] py-3.5 bg-white dark:bg-slate-900/50 backdrop-blur-md text-gray-900 dark:text-white font-semibold text-sm rounded-lg border-2 border-gray-200 dark:border-slate-700 hover:border-indigo-600 dark:hover:border-indigo-500 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all">Watch Live Demo</a>
            </div>
            <div className="flex gap-6 sm:gap-9 mt-8 pt-5 border-t border-gray-200 dark:border-slate-800">
              {[
                ...
                <div key={s.label}>
                  <div className="text-xl sm:text-2xl font-extrabold text-gray-900 dark:text-white leading-tight">{s.value}</div>
                  <div className="text-xs sm:text-sm text-gray-400 dark:text-gray-400 mt-0.5">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
          <div className="relative animate-fade-in-up-d2">
            <div className="bg-white/90 dark:bg-[#0a0f1c]/70 backdrop-blur-xl rounded-2xl shadow-xl dark:shadow-indigo-500/10 p-7 border border-gray-200 dark:border-slate-800">
              <div className="flex items-center justify-between mb-4 pb-3.5 border-b border-gray-100 dark:border-slate-800">
                <h4 className="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500 dark:text-gray-400">AI-Generated Quotation</h4>
                <span className="px-2.5 py-0.5 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 rounded-full text-xs font-semibold">AI Generated</span>
              </div>
              {[
                ...
                <div key={i} className={`flex items-center gap-2.5 py-3 ${i < 2 ? 'border-b border-gray-100 dark:border-slate-800' : ''}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${row.dot} shrink-0 shadow-[0_0_8px_rgba(0,0,0,0.5)] dark:shadow-[0_0_12px_rgba(255,255,255,0.2)]`} />
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-200 m-0">{row.name}</p>
                    <span className="text-xs text-gray-400 dark:text-gray-500 dark:text-gray-400">{row.desc}</span>
                  </div>
                  <div className="text-sm font-bold text-gray-900 dark:text-white">{row.price}</div>
                </div>
              ))}
              <div className="flex justify-between pt-3.5 mt-2.5 border-t-2 border-gray-200 dark:border-slate-800 font-bold text-base text-indigo-600 dark:text-indigo-400">
                <span>Total Amount</span>
                <span>₹53,100</span>
              </div>
            </div>
            <div className="hidden md:flex absolute -bottom-4 -left-4 bg-white dark:bg-slate-900 rounded-xl px-4 py-3 shadow-md border border-gray-200 dark:border-slate-800 items-center gap-2.5">
              <div className="w-9 h-9 bg-emerald-50 rounded-lg flex items-center justify-center text-emerald-600">
                <BarChart3 size={18} />
              </div>
              <div>
                <strong className="text-sm text-gray-900 dark:text-white">Profit Margin: 32%</strong>
      ...
      <section className="py-[72px] md:py-[72px] relative z-10" id="pain-solution">
        <div className="max-w-[1120px] mx-auto px-5">
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-1.5 px-3.5 py-1 bg-indigo-50 dark:bg-indigo-500/10 rounded-full text-xs font-semibold text-indigo-600 dark:text-indigo-400 mb-3.5 border border-indigo-100 dark:border-indigo-500/20">The Problem</div>
            <h2 className="text-[clamp(24px,3vw,34px)] font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white">Running a business shouldn't mean<br />juggling <span className="text-red-500 dark:text-red-400">5 different tools</span></h2>
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 max-w-[540px] mx-auto mt-2.5 leading-relaxed">QuoteFlow replaces your entire software stack with one unified platform. One login, one dashboard, no more context switching.</p>
          </div>
          <div className="grid md:grid-cols-2 rounded-2xl overflow-hidden border border-gray-200 dark:border-slate-800 animate-fade-in shadow-lg">
            <div className="bg-white dark:bg-slate-900 p-8 sm:p-10">
              <h3 className="text-lg font-bold mb-5 flex items-center gap-2.5 text-gray-900 dark:text-white">✗ Without QuoteFlow</h3>
              {[
                ...
                <div key={i} className="flex items-start gap-3 py-3 border-b border-gray-100 dark:border-slate-800/50 last:border-none">
                  <span className="w-6 h-6 rounded flex items-center justify-center shrink-0 text-sm font-bold bg-red-50 dark:bg-red-500/10 text-red-500 dark:text-red-400 mt-0.5">✗</span>
                  <span className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed"><strong className="text-gray-900 dark:text-gray-200">{item.strong}</strong> — {item.text}</span>
                </div>
              ))}
            </div>
            <div className="bg-indigo-50 dark:bg-indigo-900/20 p-8 sm:p-10">
              <h3 className="text-lg font-bold mb-5 flex items-center gap-2.5 text-gray-900 dark:text-white">✓ With QuoteFlow AI</h3>
              {[
                ...
                <div key={i} className="flex items-start gap-3 py-3 border-b border-indigo-100 last:border-none">
                  <span className="w-6 h-6 rounded flex items-center justify-center shrink-0 text-sm font-bold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 mt-0.5">✓</span>
                  <span className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed"><strong className="text-gray-900 dark:text-gray-200">{item.strong}</strong> — {item.text}</span>
                </div>
              ))}
            </div>
      ...
      {/* How It Works */}
      <section className="py-[72px] md:py-[72px] relative z-10 bg-[#f0ede8] dark:bg-transparent border-y border-transparent dark:border-slate-800" id="how-it-works">
        <div className="max-w-[1120px] mx-auto px-5">
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-1.5 px-3.5 py-1 bg-indigo-50 dark:bg-indigo-500/10 rounded-full text-xs font-semibold text-indigo-600 dark:text-indigo-400 mb-3.5 border border-indigo-100 dark:border-indigo-500/20">Simple Workflow</div>
            <h2 className="text-[clamp(24px,3vw,34px)] font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white">Three steps from <span className="text-red-500 dark:text-red-400">description to payment</span></h2>
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 max-w-[540px] mx-auto mt-2.5 leading-relaxed">No training required. Works exactly how you'd expect.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-6 max-w-[360px] md:max-w-none mx-auto">
            {[
              ...
              <div key={i} className={`bg-white dark:bg-slate-900 rounded-2xl p-8 border border-gray-200 dark:border-slate-800 transition-all hover:shadow-lg dark:hover:shadow-indigo-500/10 hover:border-indigo-600 dark:hover:border-indigo-500 ${i === 0 ? 'animate-fade-in-up' : i === 1 ? 'animate-fade-in-up-d1' : 'animate-fade-in-up-d2'}`}>
                <span className="inline-flex w-10 h-10 rounded-full bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-500/20 items-center justify-center text-base font-extrabold mb-3.5 shadow-sm">{step.num}</span>
      ...
      <section className="py-[72px] md:py-[72px] relative z-10" id="features">
        <div className="max-w-[1120px] mx-auto px-5">
          <div className="text-center mb-12">
            ...
            <h2 className="text-[clamp(24px,3vw,34px)] font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white">Everything your business needs<br /><span className="text-red-500">in one dashboard</span></h2>
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 max-w-[540px] mx-auto mt-2.5 leading-relaxed">12 modules working together. No integrations to set up. No data to move between tools.</p>
          </div>
          {[
            {
            ...
            <div key={idx} className={`grid md:grid-cols-2 gap-8 md:gap-12 items-center py-10 ${idx < 3 ? 'border-b border-gray-200 dark:border-slate-800' : ''} ${item.reverse ? 'md:direction-rtl' : ''} ${idx === 0 ? 'animate-fade-in-up' : 'animate-fade-in-up-d1'}`} style={item.reverse ? { direction: 'rtl' } : undefined}>
              <div style={item.reverse ? { direction: 'ltr' } : undefined}>
                ...
                <h3 className="text-xl sm:text-2xl font-bold mb-2.5 text-gray-900 dark:text-white">{item.title}</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed mb-3.5">{item.desc}</p>
                <ul className="list-none">
                  {item.benefits.map((b, i) => (
                    <li key={i} className="flex items-center gap-2.5 py-1.5 text-sm text-gray-500 dark:text-gray-400"><span className="w-1.5 h-1.5 rounded-full bg-red-500 shrink-0" />{b}</li>
                  ))}
                </ul>
              </div>
              <div style={item.reverse ? { direction: 'ltr' } : undefined} className="bg-[#f0ede8] rounded-2xl p-8 border border-gray-200 dark:border-slate-800 min-h-[240px] flex flex-col justify-center">
                <div className="grid grid-cols-2 gap-3">
                  {item.stats.map((s, i) => (
                    <div key={i} className="text-center p-4 bg-white dark:bg-slate-900 rounded border border-gray-100 dark:border-slate-800/50">
      ...
      <section className="py-[72px] md:py-[72px] relative z-10 bg-[#f0ede8]">
        <div className="max-w-[1120px] mx-auto px-5">
          <div className="text-center mb-12">
            ...
            <h2 className="text-[clamp(24px,3vw,34px)] font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white">Every tool your business needs<br /><span className="text-red-500">in one place</span></h2>
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 max-w-[540px] mx-auto mt-2.5 leading-relaxed">14 integrated modules. No third-party dependencies. One seamless experience.</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {featuresData.map((f, i) => (
              <div key={i} className="bg-white dark:bg-slate-900 rounded-2xl p-7 border border-gray-200 dark:border-slate-800 transition-all hover:shadow hover:border-indigo-600 animate-fade-in-up" style={{ animationDelay: `${(i % 6) * 50}ms` }}>
                <div className={`w-11 h-11 rounded-lg flex items-center justify-center mb-3.5 ${f.color}`}>
                  <f.icon size={20} />
                </div>
                <h3 className="text-sm font-bold mb-1.5 text-gray-900 dark:text-white">{f.title}</h3>
      ...
      <section className="py-[72px] md:py-[72px] relative z-10" id="demo">
        <div className="max-w-[1120px] mx-auto px-5">
          <div className="text-center mb-12">
            ...
            <h2 className="text-[clamp(24px,3vw,34px)] font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white">See the AI quotation generator<br /><span className="text-red-500">in action</span></h2>
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 max-w-[540px] mx-auto mt-2.5 leading-relaxed">Type any service below and watch QuoteFlow AI create a complete professional quotation instantly.</p>
          </div>
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 sm:p-9 border border-gray-200 dark:border-slate-800 max-w-[700px] mx-auto animate-fade-in">
            <div className="flex gap-1 mb-5 bg-gray-100 rounded p-1">
              <button onClick={() => setDemoTab('ai')} className={`flex-1 py-2.5 px-4 text-center rounded text-sm font-semibold transition-all ${demoTab === 'ai' ? 'bg-white dark:bg-slate-900 text-indigo-600 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:text-gray-100'}`}>AI Natural Language</button>
              <button onClick={() => { setDemoTab('wizard'); setAiResult(''); }} className={`flex-1 py-2.5 px-4 text-center rounded text-sm font-semibold transition-all ${demoTab === 'wizard' ? 'bg-white dark:bg-slate-900 text-indigo-600 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:text-gray-100'}`}>Quick Pick</button>
            </div>
            {demoTab === 'ai' ? (
              <div>
                ...
                <div className="flex gap-2.5 mb-3.5 flex-col sm:flex-row">
                  <input type="text" value={aiInput} onChange={(e) => setAiInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && simulateAIAnalysis()} placeholder='e.g. "Build a website for a school with 5 pages and SEO"' className="flex-1 px-4 py-3 rounded border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-800/50 text-sm outline-none transition-all focus:border-indigo-600 focus:bg-white dark:bg-slate-900 placeholder-gray-400" />
                ...
                {generating && (
                  <div className="bg-gray-50 dark:bg-slate-800/50 rounded p-4 border border-gray-100 dark:border-slate-800/50 text-sm leading-relaxed">
                    <div className="text-center py-3 opacity-60">Generating...</div>
                  </div>
                )}
                {!generating && aiResult && (
                  <div className="bg-gray-50 dark:bg-slate-800/50 rounded p-4 border border-gray-100 dark:border-slate-800/50 text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: aiResult }} />
                )}
              </div>
            ) : (
              <div>
                ...
                <div className="flex gap-2 flex-wrap mb-3.5">
                  {[
                    ...
                    <button key={b.type} onClick={() => { setGenerating(true); genWizard(b.type); setDemoTab('wizard'); }} className="px-3.5 py-1.5 text-sm font-semibold text-gray-900 dark:text-white bg-white dark:bg-slate-900 rounded border-2 border-gray-200 dark:border-slate-800 hover:border-indigo-600 hover:text-indigo-600 transition-all">
                      {b.label}
                    </button>
                  ))}
                </div>
      ...
      {/* Test Credentials */}
      <section className="py-[72px] md:py-[72px] relative z-10 bg-[#f0ede8] border-b border-gray-200 dark:border-slate-800">
        <div className="max-w-[1120px] mx-auto px-5">
          <div className="text-center mb-12">
            ...
            <h2 className="text-[clamp(24px,3vw,34px)] font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white">Try the full dashboard <span className="text-red-500">right now</span></h2>
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 max-w-[540px] mx-auto mt-2.5 leading-relaxed">No registration required. Use these credentials to explore every feature.</p>
          </div>
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 sm:p-9 border border-gray-200 dark:border-slate-800 max-w-[640px] mx-auto shadow animate-fade-in">
            <h3 className="text-lg font-bold mb-4.5 flex items-center gap-2.5 text-gray-900 dark:text-white">
            ...
            <div className="grid gap-2.5">
              {[
                ...
                <div key={i} className={`flex items-center justify-between px-4 py-3 rounded ${row.highlight ? 'bg-indigo-50' : 'bg-gray-50 dark:bg-slate-800/50'}`}>
                  ...
                  {row.isLink ? (
                    ...
                    <span className={`text-sm font-semibold ${row.highlight ? 'text-indigo-600' : 'text-gray-900 dark:text-white'}`}>{row.value}</span>
                  )}
                </div>
              ))}
            </div>
            <div className="mt-5 flex gap-2.5">
              ...
              <button onClick={() => navigate('/dashboard')} className="flex-1 flex justify-center py-3 px-4 bg-white dark:bg-slate-900 text-gray-900 dark:text-white font-semibold text-sm rounded-lg border-2 border-gray-200 dark:border-slate-800 hover:border-indigo-600 hover:text-indigo-600 transition-all">Preview Dashboard</button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-[72px] md:py-[72px] relative z-10">
        <div className="max-w-[1120px] mx-auto px-5">
          <div className="text-center mb-12">
            ...
            <h2 className="text-[clamp(24px,3vw,34px)] font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white">Built with <span className="text-red-500">modern technology</span></h2>
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 max-w-[540px] mx-auto mt-2.5 leading-relaxed">Enterprise-grade stack that scales with your business.</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {techData.map((t, i) => (
              <div key={i} className="bg-white dark:bg-slate-900 rounded-2xl p-7 border border-gray-200 dark:border-slate-800 text-center transition-all hover:shadow hover:border-indigo-600 animate-fade-in-up" style={{ animationDelay: `${(i % 4) * 50}ms` }}>
      ...
      <div className="max-w-[1120px] mx-auto px-5 pb-12">
        <div className="grid grid-cols-2 sm:grid-cols-4 bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 overflow-hidden animate-fade-in">
          {[
            ...
            <div key={i} className={`py-7 px-5 text-center ${i > 0 ? 'border-l border-gray-200 dark:border-slate-800 max-sm:border-l-0 max-sm:[&:nth-child(2)]:border-l max-sm:[&:nth-child(3)]:border-t max-sm:[&:nth-child(4)]:border-t' : ''}`}>
      ...
      <section className="py-[72px] md:py-[72px] relative z-10 bg-indigo-50">
        <div className="max-w-[1120px] mx-auto px-5">
          <div className="text-center mb-10">
            ...
            <h2 className="text-[clamp(24px,3vw,34px)] font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white">Smart Assessments for Your Business</h2>
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 max-w-[540px] mx-auto mt-2.5 leading-relaxed">QuoteFlow AI analyzes your quotations, invoices, and customer data to deliver actionable business insights.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className="bg-white dark:bg-slate-900 rounded-2xl p-7 border border-gray-200 dark:border-slate-800 text-center animate-fade-in-up" style={{ animationDelay: `${i * 100}ms` }}>
                ...
                <h4 className="text-base font-bold mb-2 text-gray-900 dark:text-white">{card.title}</h4>
      ...
      <section className="py-[72px] md:py-[72px] relative z-10" id="projection">
        <div className="max-w-[1120px] mx-auto px-5">
          <div className="text-center mb-12">
            ...
            <h2 className="text-[clamp(24px,3vw,34px)] font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white">Month & Year <span className="text-red-500">Cost Projection</span></h2>
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 max-w-[540px] mx-auto mt-2.5 leading-relaxed">Estimate your costs and savings with monthly and annual projections. See how much you save by choosing yearly billing.</p>
          </div>

          <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 sm:p-9 border border-gray-200 dark:border-slate-800 max-w-[800px] mx-auto shadow-sm animate-fade-in">
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="bg-gradient-to-br from-indigo-50 to-indigo-100/50 rounded-2xl p-6 border border-indigo-100">
                <div className="flex items-center gap-2.5 mb-4">
                  ...
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">Monthly Projection</h3>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-indigo-200">
                  <p className="text-xs text-gray-500 dark:text-gray-400">* All plans include 7-day free trial</p>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 rounded-2xl p-6 border border-emerald-100">
                <div className="flex items-center gap-2.5 mb-4">
                  ...
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">Yearly Projection</h3>
                ...
                <div className="mt-4 pt-3 border-t border-emerald-200">
                  <p className="text-xs text-gray-500 dark:text-gray-400">* Save up to ₹11,998/year with annual billing</p>
                </div>
              </div>
            </div>
      ...
      <section className="py-[72px] md:py-[72px] relative z-10 bg-[#f0ede8]" id="pricing">
        <div className="max-w-[1120px] mx-auto px-5">
          <div className="text-center mb-12">
            ...
            <h2 className="text-[clamp(24px,3vw,34px)] font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white">Fair pricing for <span className="text-red-500">every business</span></h2>
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 max-w-[540px] mx-auto mt-2.5 leading-relaxed">Start free. Upgrade as you grow. No hidden fees, no contracts, cancel anytime.</p>
            
            {/* Billing Cycle Toggle */}
            <div className="flex items-center justify-center gap-4.5 mb-2 mt-8">
              <span className={`text-sm font-semibold transition-colors ${billingCycle === 'monthly' ? 'text-gray-900 dark:text-white' : 'text-gray-400'}`}>Monthly Billing</span>
              <button
              ...
              >
                <span className={`w-5 h-5 bg-white dark:bg-slate-900 rounded-full transition-transform duration-200 shadow-sm ${billingCycle === 'yearly' ? 'translate-x-6' : 'translate-x-0'}`} />
              </button>
              <span className={`text-sm font-semibold transition-colors flex items-center gap-1.5 ${billingCycle === 'yearly' ? 'text-gray-900 dark:text-white' : 'text-gray-400'}`}>
                Yearly Billing
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-[360px] sm:max-w-none mx-auto">
            {pricingPlans.map((plan, i) => {
              ...
              return (
                <div key={i} className={`bg-white dark:bg-slate-900 rounded-2xl p-8 border relative transition-all hover:shadow-md hover:border-indigo-600 animate-fade-in-up ${plan.featured ? 'border-indigo-600 shadow-[0_0_0_2px_#4f46e5,0_4px_12px_rgba(0,0,0,0.08)]' : 'border-gray-200 dark:border-slate-800'}`} style={{ animationDelay: `${i * 80}ms` }}>
                  {plan.badge ? (
                  ...
                  <div className="text-[30px] font-extrabold text-gray-900 dark:text-white leading-tight mb-0.5">{displayPrice}<span className="text-sm font-normal text-gray-400">{displayPeriod}</span></div>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-4.5">{plan.desc}</p>
                  <ul className="list-none mb-5">
                    {plan.features.map((f, fi) => (
                      <li key={fi} className={`flex items-center gap-2 py-1.5 text-sm border-b border-gray-100 dark:border-slate-800/50 last:border-none ${f.included ? 'text-gray-500 dark:text-gray-400' : 'text-gray-400'}`}>
                  ...
                  <button onClick={() => navigate('/register')} className={`w-full flex justify-center py-2.5 px-4 text-sm font-semibold rounded transition-all ${plan.featured ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-white dark:bg-slate-900 text-gray-900 dark:text-white border-2 border-gray-200 dark:border-slate-800 hover:border-indigo-600 hover:text-indigo-600'}`}>
      ...
      <section className="py-[72px] md:py-[72px] relative z-10" id="testimonials">
        <div className="max-w-[1120px] mx-auto px-5">
          <div className="text-center mb-12">
            ...
            <h2 className="text-[clamp(24px,3vw,34px)] font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white">What business owners <span className="text-red-500">are saying</span></h2>
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 max-w-[540px] mx-auto mt-2.5 leading-relaxed">Join 12,000+ businesses that have transformed their operations with QuoteFlow AI.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-4">
            {testimonials.map((t, i) => (
              <div key={i} className="bg-white dark:bg-slate-900 rounded-2xl p-7 border border-gray-200 dark:border-slate-800 transition-all hover:shadow hover:border-indigo-600 animate-fade-in-up" style={{ animationDelay: `${i * 100}ms` }}>
                <div className="flex gap-0.5 mb-2.5 text-amber-500">
                  {[1, 2, 3, 4, 5].map(s => (
                    <Star key={s} size={15} className="fill-amber-500" />
                  ))}
                </div>
                <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed mb-4.5 italic">{t.text}</p>
                <div className="flex items-center gap-2.5">
        ...
        <div className="max-w-[1120px] mx-auto px-5">
          <div className="bg-indigo-600 rounded-2xl py-12 px-8 sm:px-10 text-center">
            ...
            <div className="flex gap-2.5 justify-center flex-wrap">
              <button onClick={() => navigate('/login')} className="inline-flex items-center justify-center gap-2 px-8 py-3.5 bg-white dark:bg-slate-900 text-indigo-600 font-bold text-sm rounded-lg shadow-md hover:shadow-lg transition-all hover:-translate-y-0.5">
      ...
      {/* Footer */}
      <footer className="relative z-10 bg-white dark:bg-[#0f172a] border-t border-gray-200 dark:border-slate-800 py-12 pb-6">
        <div className="max-w-[1120px] mx-auto px-5 grid sm:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1fr] gap-8 sm:gap-10">
          <div className="footer-brand">
            <Link to="/" className="flex items-center gap-2.5 text-xl font-bold text-gray-900 dark:text-white mb-2.5">
              <span className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white text-base font-extrabold">Q</span>
              QuoteFlow
            </Link>
          ...
          <div>
            <h4 className="text-xs font-semibold uppercase tracking-wider text-gray-900 dark:text-white mb-3.5">Product</h4>
            <ul className="list-none">
              {[
                { label: 'Features', href: '#features' },
                { label: 'How It Works', href: '#how-it-works' },
          ...
          <div>
            <h4 className="text-xs font-semibold uppercase tracking-wider text-gray-900 dark:text-white mb-3.5">Company</h4>
            <ul className="list-none">
              {[
          ...
          <div>
            <h4 className="text-xs font-semibold uppercase tracking-wider text-gray-900 dark:text-white mb-3.5">Support</h4>
            <ul className="list-none">
              {[
        ...
        <div className="max-w-[1120px] mx-auto px-5 mt-7 pt-4.5 border-t border-gray-200 dark:border-slate-800 flex justify-between text-sm text-gray-400 flex-wrap gap-2.5">
